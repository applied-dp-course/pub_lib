...
def compute_average_error_reconstruction_attack(
    ...
    for _ in range(num_experiments):
        subset_indices = np.random.choice(data_size, subset_size, replace=False)
...
def choose_random_subset(df):
    masks_by_size = {len(df): np.random.choice([0, 1], size=len(df))}

    def query(data):
        data_size = len(data)
        if data_size not in masks_by_size:
            masks_by_size[data_size] = np.random.choice([0, 1], size=data_size)
